...
tickLoop tiles carts =
  let
    ...
    newCarts = tickCarts tiles sortedCarts carts
  in
    if Set.size newCarts == 1
      then newCarts
      else tickLoop tiles newCarts
...
tickCarts tiles [] allCarts = allCarts
tickCarts tiles (cart: carts) allCarts =
  if Set.member cart allCarts
  then
    let
      otherCarts = Set.delete cart allCarts
      newCart = tickCart tiles cart
      withoutCollisions = removeCollisions otherCarts newCart
      newAllCarts =
        if (Set.size withoutCollisions) == (Set.size otherCarts)
          then Set.insert newCart withoutCollisions
          else withoutCollisions
    in
      tickCarts tiles carts newAllCarts
  else
    tickCarts tiles carts allCarts


removeCollisions otherCarts (Cart cartPos _ _) =
  Set.filter (\(Cart pos _ _) -> pos /= cartPos) otherCarts
