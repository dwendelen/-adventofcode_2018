...
calc logs =
  let
    ...
    maxRecord = maximumBy (comparing (\r -> snd (getInterestingMinute r))) records

    gid = getGidOfRecord maxRecord
    (minute, _) = getInterestingMinute maxRecord
  in
...
getInterestingMinute :: SleepRecord -> (Integer, Integer)
getInterestingMinute (SleepRecord _ []) = (0,0)
getInterestingMinute (SleepRecord _ cycles) =
  let
    ...
    minuteTimes = maximumBy (comparing snd) (Map.toList his)
  in
    minuteTimes
